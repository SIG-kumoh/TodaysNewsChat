...
def get_router(sio: socketio.AsyncServer):
    ...
    # TODO 시스템 이상의 권한
    @router.post("", dependencies=[Depends(auth_bearer.JWTBearer())])
    async def create_room():
        # sio 에 특정 규칙에 의해 생성된 Namespace, redis 채팅방 만들기
        return "create_room"

    # TODO 관리자 이상의 권한
    @router.delete("/{room_id}", dependencies=[Depends(auth_bearer.JWTBearer())])
    async def delete_room(room_id: str):
        # room_id 에 해당하는 Namespace를 Mongo 에 저장하고, Namespace, redis 채팅방 삭제
        return "delete_room " + room_id

    # TODO 관리자 이상의 권한
    @router.delete("/{room_id}/{chat_id}", dependencies=[Depends(auth_bearer.JWTBearer())])
